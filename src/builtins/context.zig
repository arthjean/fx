...
test "context formatting omits missing sections without extra blank lines" {
    ...
    try appendSectionFrom(&global_only, "global-rules", "/home/fx/.config/fx/AGENTS.md", "global instructions");
    try std.testing.expectEqualStrings(
        "<global-rules from=\"/home/fx/.config/fx/AGENTS.md\">\nglobal instructions\n</global-rules>",
        global_only.written(),
    );
    ...
    try appendSectionFrom(&empty, "global-rules", "/home/fx/.config/fx/AGENTS.md", "");
...
test "project instruction file cap keeps a line-safe prefix and reports source facts" {
    ...
    defer tmp.cleanup();
    try writeTestFile(tmp.dir, "home/" ++ profile_roots.test_relative_roots.config ++ "/AGENTS.md", "GLOBAL-ONE\nGLOBAL-TWO\n");
...
test "initial gather orders global ancestors workspace and exact hidden and build target scopes" {
    ...
    try writeTestFile(tmp.dir, "home/" ++ profile_roots.test_relative_roots.config ++ "/AGENTS.md", "RULE_GLOBAL");
...
test "initial gather renders an identical global and workspace source once" {
    ...
    try writeTestFile(tmp.dir, "home/" ++ profile_roots.test_relative_roots.config ++ "/AGENTS.md", "RULE_SHARED_GLOBAL_AND_WORKSPACE");
    const home = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "home");
    defer alloc.free(home);
    const workspace = try io_mod.dirRealpathAlloc(
        alloc,
        tmp.dir,
        "home/" ++ profile_roots.test_relative_roots.config,
    );
    defer alloc.free(workspace);
