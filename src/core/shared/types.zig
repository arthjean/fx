...
/// Exact usage metadata returned by a completed model stream. `model` is
/// owned by the completion carrying this value.
pub const GenerationBilling = struct {
    created_at_ms: i64,
    model: []const u8,
    total_cost: f64,
    input_tokens: u64,
...
pub const ModelCompletion = struct {
    ...
    /// Generation or resolved-model metadata was malformed or conflicting.
